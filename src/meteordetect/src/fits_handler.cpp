...
int FitsHandler::GetImageWidth()
{
	return imgwidth;
}

int FitsHandler::GetImageHeight()
{
	return imgheight;
}

bool FitsHandler::ToCvMat8(cv::Mat &mat)
{
	size_t data_size = imgwidth * imgheight;
	uint8_t *tmp_buf = new uint8_t[data_size];
	uint16_t *img_buf = imagebuf->Raw();

	for (size_t i = 0; i < data_size; i++) {
		tmp_buf[i] = (uint8_t)img_buf[i];
	}

	mat = cv::Mat(cv::Size(imgwidth, imgheight), CV_8U, tmp_buf).clone();
